...
def translate_unique_values(
    values: Iterable[str], lang: str, batch_size: int = 32, max_length: int = 128
) -> Dict[str, str]:
...
# --- MODEL 4: NLP ---
    ...
    AGENCY_NAMES = {
        ...
        "NYPD": "Nova Haven Police Department",
    ...
    if st.button("Run Full Analysis", type="primary"):
        if not description.strip():
            ...
            try:
                ...
                input_text = _clean_text(description.strip())

                # Predict agency from complaint text using Model 4
                _r_pipeline, _r_le, _, _ = load_nlp_assets()
                _predicted_agency_code = _r_le.inverse_transform(
                    [_r_pipeline.predict([input_text])[0]]
                )[0]
                # Model 5 was trained with NYPD remapped to NPD
                _M4_TO_M5 = {"NYPD": "NPD"}
                m5_agency = _M4_TO_M5.get(_predicted_agency_code, _predicted_agency_code)
                ...
                # Agency routing result (predicted by Model 4)
                _m5_agency_names = {
                    "DCWP": "Dept. of Consumer & Worker Protection",
                    "DEP":  "Dept. of Environmental Protection",
                    "DHS":  "Dept. of Homeless Services",
                ...
                _agency_display = _predicted_agency_code
                _agency_full = _m5_agency_names.get(m5_agency, _m5_agency_names.get(_predicted_agency_code, ""))
                st.info(f"**Predicted Agency:** {_agency_display} — {_agency_full}")
